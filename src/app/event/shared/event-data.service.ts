...
import { IEventInfo, Isession } from './event-info';
...
export class EventDataService {
  ...
  getEventData(): Observable<IEventInfo[]> {
    return this.http.get<IEventInfo[]>('/api/events')
      .pipe(catchError(this.handleError<IEventInfo[]>('getEventData', [])));
  }

  getEvent(id: number): Observable<IEventInfo> {
    return this.http.get<IEventInfo>('/api/events/' + id)
    .pipe(catchError(this.handleError<IEventInfo>('getEvent')));
  }

  saveEvent(event) {
    let options = {headers: new HttpHeaders({'Content-Type': 'application/json'})};
    return this.http.post<IEventInfo>('/api/events', event, options)
      .pipe(catchError(this.handleError<IEventInfo>('saveEvent')));
  }
